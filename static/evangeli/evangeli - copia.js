$(document).ready(function() {
    ...
    function sendMessage(message) {
        if (message) {
            ...
            setTimeout(() => {
                $('#chatbot-message').append(`<p class="from-bot"><span class="user">${evangeli}: </span><span class="message">${evangeli} está escribiendo... <i class="glyphicon glyphicon-pencil"></i></span></p>`);
                scrollToMessage();
            }, 500);

            setTimeout(() => {
                $('#chatbot-message .from-bot:last-child()').html(`<span class="user">${evangeli}: </span>` + respond);
                scrollToMessage();
